...
def calculating_similarity_text(definition, text_embeddings, model, terms_info, documents, uris, topics_names, topics,
                                sentiment_terms, index_uri=-1):
    ...
    for i in range(-1, end_range, -1):
        ...
        if ((index_uri >= 0) and (index_uri != similar_index)) or (index_uri < 0):
            ...
            results[uris[similar_index]] = [terms_info[similar_index][1], terms_info[similar_index][2],
                                            terms_info[similar_index][4], terms_info[similar_index][0],
                                            documents[similar_index], topic_name, rank, sentiment]
    ...
    for term in view_terms:
        if term in freq_results:
            ...
            for year in freq_results[term]:
                normed_results[term][year] = (freq_results[term][year] * len(term.split())) / float(
                    publication[int(year)][2])
